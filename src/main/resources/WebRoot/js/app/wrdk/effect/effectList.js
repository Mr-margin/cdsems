...
function initTable() {  
    ...
    $('#dataTable').bootstrapTable({
        url: '/cdsems/ContaminatedTableController/selectEffectTable',  //请求后台的URL（*）
        method: 'post',                     //请求方式（*）
		...
		onLoadSuccess: function(data){
		 ...
		 columns: [
	        ...
	        {
	            field: 'effectEvaluationTitle',
	            title: '标题',
	            align:'center',
	        },
	        {
	            field: 'effectEvaluationWebsite',
	            title: '公示网址',
	            align:'center',
	        },
	        {
	            field: 'effectEvaluationUnit',
	            title: '编制单位',
	            align:'center',
	        },
	        {
	            field: 'effectEvaluationTime',
	            title: '编制时间',
	            align:'center',
				formatter : function(value,row,index){
                    var date =  new Date(row.effectEvaluationTime);
                    if( row.effectEvaluationTime == null || row.effectEvaluationTime == "" || row.effectEvaluationTime == undefined ) return "";
	        ...
	        },
	        {
	            ...
	            formatter:function(value,row,index){
	                ...
	                var e = '<a class="btn btn-origin btn-xs eleNone xgpg_update" title="编辑"  onclick="updateEffect('+index+')"><i class="fa fa-pencil-square-o"></i></a> ';
	                var d = '<a class="btn btn-danger btn-xs eleNone xgpg_detele" title="删除"  onclick="delEffect('+row.eid+')"><i class="fa fa-trash"></i></a> ';
	               // return s+m+e+d;
	                return e+d;
	            },
	        }
 ...
 */
function addEffect(){
	window.location.href="#effectAdd";
}
/**
 * 编辑效果评估
 */
function updateEffect(index){
	...
	window.location.href="#effectUpdate";
}
/**
 * 删除效果评估
 */
function delEffect(cid){
	delInfo(cid);
}
//删除效果评估
function delData(cid){
	var json = {cid:cid};
	ajaxPost('/' + projectName + '/ContaminatedTableController/deleteEffect', json).done(function(result){
		if(result.status == 0){
			sucInfo("删除成功！");
			initTable();
		}else{
        ...
        confirmButtonColor: "#ec6c62"
		  ...
		  if(result == true) {
			  ...
			  var arr=[];
		        ...
		        ajaxPost('/' + projectName + '/ContaminatedTableController/deleteEffectBatch', {data: arr.join(",")}).done(function (data) {
		            if (data.status == 0) {
		                swal("成功删除选中效果评估", "", "success");
		                initTable(); 
		            } else {
		                swal("删除效果评估", "", "error");
		            }
