...
function getEffect(row){
	...
	if ( row.effectEvaluationTime == "" || row.effectEvaluationTime == null || row.effectEvaluationTime == undefined ) {
        $("#effectEvaluationTime").val("");
    } else  {
        var y = 1900+date.getYear();
        var m = "0"+(date.getMonth()+1);
        var d = "0"+date.getDate();
        $("#effectEvaluationTime").val(y+"-"+m.substring(m.length-2,m.length)+"-"+d.substring(d.length-2,d.length));
    }
  }
...
function updateEffect(){
	if($("#dataForm").valid()){
        ...
        $.ajax({
            type:'POST',
            url: '/' + projectName + '/ContaminatedLandController/insertTEffectContaminated',
            data:formData,
            contentType:false,
            ...
            success:function(data){
                if (data.status == 0) {
                    sucInfo("修改成功！");
                    window.location.href = "#effectList";
                } else {
                    errInfo("修改失败！");
                }
            },
