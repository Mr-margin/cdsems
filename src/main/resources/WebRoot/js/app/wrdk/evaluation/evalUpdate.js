...
function getInvest(row){
	...
	if ( row.riskAssessmentTime == "" || row.riskAssessmentTime == null || row.riskAssessmentTime == undefined ){
        $("#preliminaryCompilingTime").val("");
    } else {
        var y = 1900+date.getYear();
        var m = "0"+(date.getMonth()+1);
        var d = "0"+date.getDate();
        $("#riskAssessmentTime").val(y+"-"+m.substring(m.length-2,m.length)+"-"+d.substring(d.length-2,d.length));
    }
}
...
function updateEv(){
	if($("#dataForm").valid()){
        ...
        $.ajax({
            type:'POST',
            url: '/' + projectName + '/ContaminatedLandController/insertTAssessmentContaminated',
            data:formData,
            contentType:false,
