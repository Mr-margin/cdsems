...
function getInvest(row){
	...
	if ( row.detailedCompilingTime == "" || row.detailedCompilingTime == null || row.detailedCompilingTime == undefined ) {
        $("#detailedCompilingTime").val("");
    } else {
        var y = 1900+date.getYear();
        var m = "0"+(date.getMonth()+1);
        var d = "0"+date.getDate();
        $("#detailedCompilingTime").val(y+"-"+m.substring(m.length-2,m.length)+"-"+d.substring(d.length-2,d.length))
    }
}
/**
 * 添加
...
function addInv(){
	if($("#dataForm").valid()){
        ...
        $.ajax({
            type:'POST',
            url: '/' + projectName + '/ContaminatedLandController/insertTDtailedContaminated',
            data:formData,
            contentType:false,
            ...
            success:function(data){
                if (data.status == 0) {
                    sucInfo("修改成功！");
                    window.location.href = "#deInvestList";
                } else {
                    errInfo("修改失败！");
                }
            },
