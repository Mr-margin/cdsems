...
function initTable() {  
    ...
    $('#dataTable').bootstrapTable({
        url: '/cdsems/ContaminatedTableController/selectDetailedTable',  //请求后台的URL（*）
        method: 'post',                     //请求方式（*）
		...
		onLoadSuccess: function(data){
		 ...
		 columns: [
	        ...
	        {
	            field: 'detailedWebsite',
	            title: '公示网址',
	            align:'center',
	        },
	        {
	            field: 'detailedCompilingUnit',
	            title: '编制单位',
	            align:'center',
	        },
	        {
	            field: 'detailedCompilingTime',
	            title: '编制时间',
	            align:'center',
				formatter:function(value,row,index){
                    var date =  new Date(row.detailedCompilingTime);
                    if ( row.detailedCompilingTime == "" || row.detailedCompilingTime == null || row.detailedCompilingTime == undefined ) return "";
	        ...
	        },
	        {
	            ...
	            formatter:function(value,row,index){
	                ...
	                var e = '<a class="btn btn-origin btn-xs eleNone xxdc_update" title="编辑"  onclick="updateInv('+index+')"><i class="fa fa-pencil-square-o"></i></a> ';
	                var d = '<a class="btn btn-danger btn-xs eleNone  xxdc_detele" title="删除"  onclick="delInv('+row.did+')"><i class="fa fa-trash"></i></a> ';
	               // return s+m+e+d;
	                return e+d;
	            },
	        }
 ...
 */
function addInv(){
	window.location.href="#deInvestAdd";
}
/**
 * 编辑详细调查
 */
function updateInv(index){
	//将选中行数据存入session当中
	//获取表单数据
	var rows = $("#dataTable").bootstrapTable('getData');
	//将选中行数据存入session当中
	sessionStorage["row"]=JSON.stringify(rows[index]);
	window.location.href="#deInvestUpdate";
}
/**
 * 删除详细调查
 */
...
function delData(cid){
	var json = {cid:cid};
	ajaxPost('/' + projectName + '/ContaminatedTableController/deleteDetailed', json).done(function(result){
		if(result.status == 0){
			sucInfo("删除成功！");
			initTable();
		}else{
        ...
        confirmButtonColor: "#ec6c62"
		  ...
		  if(result == true) {
			  ...
			  var arr=[];
		        ...
		        ajaxPost('/' + projectName + '/ContaminatedTableController/deleteDetailedBatch', {data: arr.join(",")}).done(function (data) {
		            if (data.status == 0) {
		                swal("成功删除选中详细调查", "", "success");
		                initTable(); 
		            } else {
		                swal("删除详细调查", "", "error");
		            }
