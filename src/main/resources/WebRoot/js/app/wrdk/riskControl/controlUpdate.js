...
function getInvest(row){
    ...
    if( row.riskControlTime == "" || row.riskControlTime == undefined || row.riskControlTime == null ) {
        $("#riskControlTime").val("")
    } else {
        var y = 1900+date.getYear();
        var m = "0"+(date.getMonth()+1);
        var d = "0"+date.getDate();
        $("#riskControlTime").val(y+"-"+m.substring(m.length-2,m.length)+"-"+d.substring(d.length-2,d.length))
    }
  }
...
function updateCon(){
	if($("#dataForm").valid()){
		 var formData = new FormData();
	        ...
	        $.ajax({
	            type:'POST',
	            url: '/' + projectName + '/ContaminatedLandController/insertTControlContaminated',
	            data:formData,
	            contentType:false,
