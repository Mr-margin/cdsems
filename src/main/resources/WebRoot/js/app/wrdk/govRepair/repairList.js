...
function initTable() {  
    ...
    $('#dataTable').bootstrapTable({
        url: '/cdsems/ContaminatedTableController/selectRepairTable',  //请求后台的URL（*）
        method: 'post',                     //请求方式（*）
		...
		onLoadSuccess: function(data){
		 ...
		 columns: [
	        ...
	        {
	            field: 'treamentRepairTitle',
	            title: '标题',
	            align:'center',
	        },
	        {
	            field: 'treamentRepairWebsite',
	            title: '公示网址',
	            align:'center',
	        },
	        {
	            field: 'treamentRepairUnit',
	            title: '编制单位',
	            align:'center',
	        },
	        {
	            field: 'treamentRepairTime',
	            title: '编制时间',
	            align:'center',
                formatter:function(value,row,index){
                    var date =  new Date(row.treamentRepairTime);
                    if ( row.treamentRepairTime == null || row.treamentRepairTime == "" || row.treamentRepairTime ==undefined) return "";
	        ...
	        },
	        {
	            ...
	            formatter:function(value,row,index){
	                ...
	                var e = '<a class="btn btn-origin btn-xs eleNone xfzl_update" title="编辑"  onclick="updateRepair('+index+')"><i class="fa fa-pencil-square-o"></i></a> ';
	                var d = '<a class="btn btn-danger btn-xs eleNone xfzl_detele" title="删除"  onclick="delRepair('+row.tid+')"><i class="fa fa-trash"></i></a> ';
	               // return s+m+e+d;
	                return e+d;
	            },
	        }
...
function updateRepair(index){
	...
	window.location.href="#repairUpdate";
}
/**
 * 删除治理修复
 */
function delRepair(cid){
	delInfo(cid);
}
//删除治理修复
function delData(cid){
	var json = {cid:cid};
	ajaxPost('/' + projectName + '/ContaminatedTableController/deleteRepair', json).done(function(result){
		if(result.status == 0){
			sucInfo("删除成功！");
			initTable();
		}else{
        ...
        confirmButtonColor: "#ec6c62"
		  ...
		  if(result == true) {
			  ...
			  var arr=[];
		        ...
		        ajaxPost('/' + projectName + '/ContaminatedTableController/deleteRepairBatch', {data: arr.join(",")}).done(function (data) {
		            if (data.status == 0) {
		                swal("成功删除选中治理修复", "", "success");
		                initTable(); 
		            } else {
		                swal("删除治理修复", "", "error");
		            }
