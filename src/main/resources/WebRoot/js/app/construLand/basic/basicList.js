...
function initTable() {  
    ...
    $('#dataTable').bootstrapTable({
        url: '/cdsems/BuLidingLandController/selectBuLidingTable',  //请求后台的URL（*）
        method: 'post',                     //请求方式（*）
		 ...
		 columns: [
	        ...
	        },
	        {
	            ...
	            formatter:function(value,row,index){
	                ...
	                var e = '<a class="btn btn-origin btn-xs eleNone jsyd_update" title="编辑"  onclick="Update('+index+')"><i class="fa fa-pencil-square-o"></i></a> ';
	                var d = '<a class="btn btn-danger btn-xs eleNone jsyd_detele" title="删除"  onclick="del('+row.lid+')"><i class="fa fa-trash"></i></a> ';
	                //return s+m+e+d;
	                return e+d;
	            },
	        }
...
//删除地块
function delData(lcid){
	var json = {lid:lcid};
    swal({
        title: "确认删除",
        text: "删除建设用地基础信息时会同时删除与其关联的农用地调查信息数据，确定删除此条数据？",
        type: "warning",
        ...
        if(result == true) {
            $.ajax({
                url:'/' + projectName + '/BuLidingLandController/deleteBuLiding',
                type:"post",
...
function Dele() {
	...
	swal({
        title: "确认删除",
        text: "删除建设用地信息同时会删除与其关联的调查信息数据，确认删除选中的数据？",
        type: "warning",
        showCancelButton: true,
        closeOnConfirm: false,
        cancelButtonText: "取消",
        ...
        confirmButtonColor: "#ec6c62"
		  ...
		  if(result == true) {
			  ...
			  var arr=[];
		        ...
		        ajaxPost('/' + projectName + '/BuLidingLandController/deleteBuLidingBatch', {data: arr.join(",")}).done(function (data) {
		            if (data.status == 0) {
...
function expExcel(){
	swal({
        ...
        confirmButtonColor: "#ec6c62"
		  ...
		  if(result == true) {
              var url = '/' + projectName + '/BuLidingLandController/exportBuLiding?massifName='+$("#massifName").val()+'&countyCode='+$("#countyCode").val()+'&massifAddress='+$("#massifAddress").val()+'&organizationCode='+$("#organizationCode").val();
