...
function addNYDPoint(){
	require(["esri/layers/GraphicsLayer","esri/InfoTemplate","esri/layers/ArcGISDynamicMapServiceLayer",
	         "esri/graphic","esri/geometry/Point","esri/symbols/PictureMarkerSymbol"
	],function(GraphicsLayer,InfoTemplate,ArcGISDynamicMapServiceLayer,
			Graphic,Point,PictureMarkerSymbol
	){
		var layer = new ArcGISDynamicMapServiceLayer("http://192.168.1.24:6080/arcgis/rest/services/cdsems/guojiexian/MapServer");
		map.addLayer(layer);
		var template = new InfoTemplate();
		template.setTitle("农用地信息")
		template.setContent(getContent);
		var graphicsLayer = new GraphicsLayer({"id":"qiye",infoTemplate:template});
		map.addLayer(graphicsLayer);
		var pictureMarkerSymbol = new PictureMarkerSymbol('img/nyd/marker1.png', 48, 48);
		var pictureMarkerSymbol2 = new PictureMarkerSymbol('img/nyd/marker2.png', 48, 48);
		var pictureMarkerSymbol3 = new PictureMarkerSymbol('img/nyd/marker3.png', 48, 48);
		$.getJSON("data/nyd.json",function(data){
			var industryData = data.data;
			for (var i = 0; i < industryData.length; i++) {
				var itemData = industryData[i];
				if(itemData.x&&itemData.y){
					...
					var point = new Point(itemData.x,itemData.y);
					var graphic;
					if("1"==itemData.type){
						graphic = new Graphic(point,pictureMarkerSymbol,attriubtes);
					}else if("2"==itemData.type){
						graphic = new Graphic(point,pictureMarkerSymbol2,attriubtes);
					}else if("3"==itemData.type){
						graphic = new Graphic(point,pictureMarkerSymbol3,attriubtes);
					}
					graphicsLayer.add(graphic);
				}
			}
